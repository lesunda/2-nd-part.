...
void pagrindinio_failo_kurimas(int studentu_kiekis)
{
	...
	std::uniform_int_distribution <int> Intervalas(1, 10), Suma(10, 20);

	int studentu_skaicius = studentu_kiekis,
		rezultatu_skaicius = Suma(e);

	std::ofstream failas;
	failas.open("Studentai_" + std::to_string(studentu_skaicius) + ".txt");

	for (size_t j = 1; j <= studentu_skaicius; j++)
	{
		failas << "Vardas" + std::to_string(j) << " Pavarde" + std::to_string(j) << " " << Intervalas(e);

		std::vector <int> Results;
		std::vector <int>::iterator iteratorius;

		for (size_t k = 0; k < rezultatu_skaicius; k++)
			Results.push_back(Intervalas(e));
...
//---------------------------------------------------------------------------------------------------------------------------
double Studentas::GALPAZ(double (*k)(std::vector<int>))
{
	return GALPAZ(Egz, k(Pazymiai));
}
//---------------------------------------------------------------------------------------------------------------------------
